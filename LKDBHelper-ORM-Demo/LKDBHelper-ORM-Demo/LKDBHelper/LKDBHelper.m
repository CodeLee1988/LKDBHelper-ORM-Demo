...
- (BOOL)insertBase:(NSObject *)model
{
    ...
    // 如果需要插入后自动更新主键 by weixian
    if ([model.class autoIncreatPrimaryKey]) {
        LKModelInfos *infos = [model.class getModelInfos];
        NSArray *primaryKeys = infos.primaryKeys;
        if (primaryKeys.count > 0) {
            NSString *pk = [primaryKeys objectAtIndex:0];
            [model setValue:[NSNumber numberWithInteger:model.rowid] forKeyPath:pk];
        }
    }
...
//#pragma mark - 自定义
//- (void)startCreatDabase{
//     NSString *filePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"BBContactDatabase.sqlite"];
////    [self initWithDBName:@"BBContactDatabase.sqlite"];
//    [self initWithDBPath:filePath];
//
//}
//
//- (void)move{
...
//    [[NSFileManager defaultManager] createDirectoryAtPath: [NSString stringWithFormat:@"%@/myFolder", kDocumentsPath] attributes:nil];
//#pragma clang diagnostic pop
//    NSString *databasePath = kDatabasePath;
//    NSFileManager *fileMgr = [NSFileManager defaultManager];
//    NSString *newFilePath = [kDocumentsPath stringByAppendingPathComponent:@"myFolder"];
//    NSLog(@"%@", newFilePath);
//    NSString *databasePath2 = [newFilePath stringByAppendingPathComponent:@"BBContactDatabase2.sqlite"];
//    [fileMgr moveItemAtPath:databasePath toPath:databasePath2 error:NULL];
//}
